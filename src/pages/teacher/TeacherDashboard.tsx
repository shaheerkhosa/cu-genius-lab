...
const TeacherDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (headerRef.current) {
      tl.fromTo(headerRef.current, { opacity: 0, y: -40 }, { opacity: 1, y: 0, duration: 0.7, ease: "power3.out" });
    }
    if (promptsRef.current) {
      tl.fromTo(promptsRef.current.children, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: 0.5, stagger: 0.15, ease: "power2.out" }, "-=0.4");
    }
    if (inputRef.current) {
      tl.fromTo(inputRef.current, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.5, ease: "power2.out" }, "-=0.2");
    }
  }, []);

  const handleSubmit = (message: string) => {
    if (message.trim()) {
      navigate("/teacher/chat", { state: { initialMessage: message.trim() } });
    }
  };
