#!/usr/bin/env node
// Generate a Northbridge University seed migration from the spec.
...
const buildings = BUILDINGS.map(b => ({
  ...
  address: `Northbridge University Campus, University Road, ${UNIVERSITY.city}`,
}));
  ...
  for (const dept of c.departments) {
    for (let i = 0; i < facultyPerDept; i++) {
      ...
      faculty.push({
        ...
        bio: `${title} of ${dept} in the ${c.name}. Research interests: ${research.join(', ')}. Joined Northbridge in ${2000 + Math.floor(rand() * 25)}.`,
...
const lines = [];
lines.push(`-- =============================================================================`);
lines.push(`-- Northbridge University seed data (Session 1).`);
